...
class PhoneNumber(Validator):
    ...
    def __init__(self):
        pattern = r'^\(?(\+?84|0)(1[2689]|[89])\d\)?([ .-]?)([0-9]{3})\3([0-9]{4})$'
...
class Password(Validator):
    ...
    def __init__(self):
        ...
        self.not_message = "Invalid new password, password not needed:contains at least 8 characters," \
                           "contains at least one number,at least 1 lowercase letter (a-z), " \
                           "at least 1 capital letter (A-Z),contains only 0-9a-zA-Z. pattern: %s"
        self.compiled = re.compile(pattern)
