...
def find_boxes(boxes_mask: np.ndarray, mode: str= 'min_rectangle', min_area: float=0.2,
    ...
    if n_max_boxes == 1:
        ...
        return [fb for i, fb in enumerate(found_boxes) if i <= n_max_boxes]


def find_rect_boxes(boxes_mask: np.ndarray, min_area: float=0.01):
    polygons = np.array(find_boxes(boxes_mask, min_area=min_area))
    boxes = [(np.amin(box, 0), np.amax(box, 0)) for box in polygons]
    return boxes
