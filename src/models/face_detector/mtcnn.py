...
def pnet_serving(image, tf_serving='localhost:9500'):
    channel = grpc.insecure_channel(tf_serving, options=[('grpc.max_send_message_length', MAX_MESSAGE_LENGTH),
                                                         ('grpc.max_receive_message_length', MAX_MESSAGE_LENGTH)])
    stub = PredictionServiceStub(channel)

    request = predict_pb2.PredictRequest()

    request.model_spec.name = 'face-detector'
    ...
    return [MakeNdarray(result.result().outputs['result1']),
            MakeNdarray(result.result().outputs['result2'])]


def onet_serving(image, tf_serving='localhost:9500'):
    channel = grpc.insecure_channel(tf_serving)
    stub = PredictionServiceStub(channel)
    ...
    return [MakeNdarray(result.result().outputs['result1']),
            MakeNdarray(result.result().outputs['result2']),
            MakeNdarray(result.result().outputs['result3'])]


def rnet_serving(image, tf_serving='localhost:9500'):
    channel = grpc.insecure_channel(tf_serving)
    stub = PredictionServiceStub(channel)
...
def detect_face(img, minsize=20, threshold=None, factor=0.709, tf_serving='localhost:9500'):
    """
    Returns:
    total_boxes: [[left, top, right, bottom, prob], ...]
    points: [[nose0, eye_right0, eye_left0, mouth_right0, mouth_left0, nose1, eye_right1, eye_left1, mouth_right1, mouth_right1], ...] 
    """
    ...
    for scale in scales:
        ...
        out = pnet_serving(img_y, tf_serving)
    ...
    if numbox > 0:
        ...
        out = rnet_serving(tempimg1, tf_serving)
    ...
    if numbox > 0:
        ...
        tempimg = (tempimg - 127.5) * 0.0078125
        tempimg1 = np.transpose(tempimg, (3, 1, 0, 2))
        out = onet_serving(tempimg1, tf_serving)
        out0 = np.transpose(out[0])
        out1 = np.transpose(out[1])
