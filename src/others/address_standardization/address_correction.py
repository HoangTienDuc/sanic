...
class AddressCorrection:
    ...
    def __init__(self, cost_dict_path=None, provinces_path=None, districts_path=None, wards_path=None, underwards_path=None):
        ...
        with open(provinces_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                self.provinces.extend(entites)
                self.no_abbr_provinces.append(entites[0])
                for province in entites:
                    self.s_provinces.update({province:entites[0]})

        with open(districts_path, 'r', encoding='utf-8') as f:
            for line in f:
                entity = line.strip()
                districts, provinces = entity.split('\t')
                districts = districts.strip().split('|')
                ...
                for district in districts:
                    self.s_districts[(self.s_provinces[provinces[0]], district)].extend([districts[0]])
        ...
        self.no_abbr_provinces.extend(['hcm', 'tt huế'])
        self.no_abbr_provinces = tuple(set(self.no_abbr_provinces))

    def correct(self, phrase, correct_phrases, nb_candidates=2, distance_threshold=40, check_exact_digit=True):
        ...
        for correct_phrase in correct_phrases:
            if abs(len(phrase) - len(correct_phrase)) >= max_diff_length:
                continue
            if check_exact_digit and extract_digit(correct_phrase) != extract_digit(phrase):
                distance = 100
            else:
    ...
    def _district_correction(self, tokens, prefix_province, province,
        ...
        for district_index in range(max(0, current_province_index - 4), current_province_index):
            ...
            for district, distance_district in district_candidates:
                tmp_districts = self.s_districts[(province, district)]
                for stand_district in tmp_districts:
                    stand_districts.append((stand_district, distance_district))

            for district, distance_district in stand_districts:
                if distance_district > 20:
                    continue
        ...
        for index_province in range(max(0, nb_of_tokens - 4), nb_of_tokens):
            ...
            for province, distance_province in province_candidates:
                if province is not None:
                    province = self.s_provinces[province]
    ...
    def address_correction(self, address, correct_th=50):
        ...
        if distance_result <= correct_th:
            ...
            if nb_of_comma > 2:
                prefix_number = ('tdp', 'ấp', 'phố', 'số', 'đội', 'xóm', 'khu', 'ngách', 'đường', 'tổ', 'ngõ', 'phường', 'khóm', 'thôn', 'kp', 'tt', 't t', 'tòa', 'nơ', 'chộ', 'c hộ', 'lộ', 'lô')
                tokens = result.split()
                for i in range(1, min(5, len(tokens) - 1)):
                    ...
                    if (not have_comma and not tokens[i].isalpha()) or (have_comma and not tokens[i][:-1].isalpha()):
                        corrected_token = self.correct(tokens[i-1], prefix_number, nb_candidates=1, distance_threshold=16)[0]
    ...
    def province_correction(self, province_inp, correct_th=100):
        '''
        Correct province only
        Args:
            province (str): province need to correct
            correct_th (int): correct distance threshold
        Returns:
            (str, int): corrected province and distance between corrected and input
        '''
        result_distance = correct_th
        result = None
        province_inp = province_inp.lower()
        tokens = province_inp.split()
        # check prefix
        province_prefix = None
        prefix_candidiates = self.correct(tokens[0], ['tỉnh', 'thành phố', 'tp'], distance_threshold=30, nb_candidates=1, check_exact_digit=False)
        prefix_pos = -1
        province_cand = set(self.no_abbr_provinces)
        if prefix_candidiates[0][0] is not None and prefix_candidiates[0][1]/len(tokens) < 6:
            province_prefix = prefix_candidiates[0][0]
            prefix_pos = 0
            if province_prefix == 'tỉnh':
                province_cand -= {'hcm', 'hà nội', 'hải phòng', 'đà nẵng', 'hồ chí minh'}
        else:
            prefix_candidiates = self.correct(tokens[0], ['thành phố'], distance_threshold=20, nb_candidates=1)
            if prefix_candidiates[0][0] is not None:
                province_prefix = 'thành phố'
                prefix_pos = 1
        phrase = ' '.join(tokens[prefix_pos+1:])
        province_candidates = self.correct(phrase, province_cand, distance_threshold=100, nb_candidates=1, check_exact_digit=False)
        province_candidate = province_candidates[0]
        province = province_candidate[0]
        result_distance = province_candidate[1]
        province_candidates = self.correct(' '.join(tokens), province_cand, distance_threshold=100, nb_candidates=1, check_exact_digit=False)
        if province_candidate[1] > province_candidates[0][1]:
            province = province_candidates[0][0]
            province_prefix = ''
            result_distance = province_candidates[0][1]
        if province is not None:
            if province_prefix:
                result = province_prefix + ' ' + province
            else:
                result = province
        else:
            result = province_inp
    ...
    def extract_address_entity(self, address):
        address, th = self.address_correction(address.lower())
        result = {}
        province_address = ''
        district_address = ''
        ward_address = ''
        ...
        if "," in address:
            ...
            province2 = ' '.join(arr_address[-2:])
            province3 = ' '.join(arr_address[-3:])
            province4 = ' '.join(arr_address[-4:])

            if province2 in self.s_provinces:
                province_address = self.s_provinces[province2].replace('_', ' ').upper().strip()
                street_address = ' '.join(arr_address[:-2])
            elif province3 in self.s_provinces:
                province_address = self.s_provinces[province3].replace('_', ' ').upper().strip()
                street_address = ' '.join(arr_address[:-3])
            elif province4 in self.s_provinces:
                province_address = self.s_provinces[province4].replace('_', ' ').upper().strip()
                street_address = ' '.join(arr_address[:-4])
            else:
                province_address = address.replace('_', ' ').upper().strip()
                street_address = ''

        result["province"] = str(province_address).upper().strip()
        ...
        try:
            prefix_province = ['TỈNH ', 'THÀNH PHỐ ', 'T ', 'TP ', 'T P ']
            for i in prefix_province:
                if result["province"].find(i) != -1:
                    if i == 'T ' and result['province'].split()[0] != i.strip():
                        continue
                    result["province"] = result["province"].replace(i, '')

            prefix_district = ['QUẬN ', 'HUYỆN ', 'THÀNH PHỐ ', 'THỊ XÃ ', 'Q ', 'H ', 'TX ', 'T X ', 'TP ', 'T P ']
            if result["district"].replace(' ', '').isalpha():
                for i in prefix_district:
                    if result["district"].find(i) != -1:
                        if i == 'H ' and result['district'].split()[0] != i.strip():
                            continue
                        result["district"] = result["district"].replace(i, '')
            else:
                if result["district"].find('Q ') != -1:
                    result["district"] = result["district"].replace('Q ', '')
        except:
