...
class AddressCorrection:
    ...
    def address_correction(self, address, correct_th=50):
        ...
        if distance_result <= correct_th:
            ...
            if nb_of_comma > 2:
                prefix_number = (
                'tdp', 'ấp', 'phố', 'số', 'đội', 'xóm', 'khu', 'ngách', 'đường', 'tổ', 'ngõ', 'phường', 'khóm', 'thôn',
                'kp', 'tt', 't t', 'tòa', 'nơ', 'chộ', 'c hộ', 'lộ', 'lô')
                tokens = result.split()
                for i in range(1, min(5, len(tokens) - 1)):
                    ...
                    if (not have_comma and not tokens[i].isalpha()) or (have_comma and not tokens[i][:-1].isalpha()):
                        corrected_token = \
                        self.correct(tokens[i - 1], prefix_number, nb_candidates=1, distance_threshold=16)[0]
    ...
    def province_correction(self, province_inp, correct_th=100):
        ...
        prefix_candidiates = self.correct(tokens[0], ['tỉnh', 'thành phố', 'tp'], distance_threshold=30,
                                          nb_candidates=1, check_exact_digit=False)
        prefix_pos = -1
        ...
        province_candidates = self.correct(phrase, province_cand, distance_threshold=100, nb_candidates=1,
                                           check_exact_digit=False)
        province_candidate = province_candidates[0]
        province = province_candidate[0]
        result_distance = province_candidate[1]
        province_candidates = self.correct(' '.join(tokens), province_cand, distance_threshold=100, nb_candidates=1,
                                           check_exact_digit=False)
